...
| The website constructs the authorization URL with `client_id`, `redirect_uri`, `scope`, and PKCE params | `App.js` → `handleLogin()` generates PKCE challenge and constructs the URL. |

**Real-World (Google) Authorization URL:**
```http
...
| Google validates credentials against **Google Accounts** (millions of users, MFA, security features) | `SecurityConfig.users()` validates against `InMemoryUserDetailsManager` with **2 hardcoded users**:<br><br>• **Username:** `user` \| **Password:** `password` \| **Role:** `ROLE_USER`<br>• **Username:** `admin` \| **Password:** `admin` \| **Role:** `ROLE_ADMIN` |
...
| Real-World (Google) | Demo |
| :--- | :--- |
| Google shows a **consent screen**: *"myapp.com wants to: View your email address, View your profile info"* | The Spring AS shows a **consent page** (because `requireAuthorizationConsent=true` in `ClientSettings`):<br><br>`/oauth2/authorize consent page:`<br>`"Demo Client requests the following scopes:`<br>` ☐ openid  ☐ profile"` |
...
| Real-World (Google) | Demo |
| :--- | :--- |
| Google redirects the browser back to the third-party site (see response below). | The Spring AS redirects back to the frontend. |
...
**Real-World (Google) Redirect Response:**
```http
302 Redirect
...
**Real-World (Google) Token Request:**
```http
...
| Google responds with tokens (see JSON below). | Spring AS responds with tokens. |

**Real-World (Google) Token Response:**
```json
{
...
```
* The `access_token` is opaque to Google (could be JWT or reference token).
* The `id_token` is a **JWT** containing user claims (`sub`, `name`, `email`, `picture`).

**Demo Token Response Details:**
* The demo's `access_token` is a **signed JWT** (RS256 with the local RSA key).
* The `id_token` is a **JWT** containing `sub` (user's login name), `iss` (`http://localhost:8085`), `aud` (`demo-client`).
...
| Real-World (Google) | Demo |
| :--- | :--- |
| Google returns user info:<br><br>```json<br>{<br>  "id": "12345",<br>  "email": "user@gmail.com",<br>  "name": "John Doe",<br>  "picture": "https://..."<br>}<br>``` | Demo returns a simple greeting. |
...
| Real-World (Google) | Demo |
| :--- | :--- |
| When the access token expires (3600s), the backend uses the `refresh_token` to get a new one:<br><br>```http<br>POST https://oauth2.googleapis.com/token<br>grant_type=refresh_token&<br>refresh_token=1//0g...&<br>client_id=738...apps.googleusercontent.com<br>``` | The demo supports `REFRESH_TOKEN` grant type — the `OAuth2FlowExample.java` includes it in its provider chain:<br><br>```java<br>OAuth2AuthorizedClientProviderBuilder.builder()<br>    .authorizationCode()<br>    .refreshToken()   // ← enabled<br>    .clientCredentials()<br>    .build();<br>``` |
...
| `email` | `user@gmail.com` | *(not present — no `email` scope configured)* |
| `name` | `John Doe` | *(not present — profiles depend on UserDetails)* |
