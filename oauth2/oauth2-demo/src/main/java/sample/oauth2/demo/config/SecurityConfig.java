...
public class SecurityConfig {
    ...
    public SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
        ...
        http
            .authorizeHttpRequests(auth -> auth
                .requestMatchers("/", "/error", "/webjars/**", "/login/**", "/oauth2/**", "/.well-known/**", "/h2-console/**", "/db/**").permitAll()
    ...
    public UserDetailsService users(PasswordEncoder passwordEncoder) {
        // In-memory users for interactive login during the demo (used on the Authorization Server login page)
    ...
    public ClientRegistrationRepository clientRegistrationRepository() {
        ...
        // against the local Authorization Server. Values mirror the RegisteredClient stored in the DB.
